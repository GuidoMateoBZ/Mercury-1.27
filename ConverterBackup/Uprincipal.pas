...
uses
  ...
  Dialogs, Menus, ComCtrls, shellapi, AppEvnts, ToolWin, PuertoSerie,
  ImgList, StdCtrls, Grids, Buttons, UUtiles, UEquipo, Mask, UPresentacion,
  USensor, UPreferencias, ExtCtrls, Gauges, QDialogs, UDataModule, UCalculoParam,
  UConexionesRemotas, UConexionAuto, UConfiguracionInternet, UServerSocket;
...
var
  ...
  Server        : TServer;                  // Objeto que administra la conexión por Internet
...
begin
  ...
  // Cargo la configuración del programa guardada en el Archivo INI
  FPresentacion.LMensaje.Caption := 'Cargando Configuración...';
  FPresentacion.Repaint;
  Mercury.CargarConfig;
  Retardo(200);

  // Cargo la config a la aplicación, a las vantanas, etc
  CargarDatosConfig;

  // Cargo la configuración de las Comuicaciones Telefónicas guardada en el Archivo INI
  ...
  // Cargo la lista de puertos serie en el menú
  mConexionMan.Clear;
  ...
  // Inicializo el Tipo de Comunicación SERIE O CELULAR
  ...
  // Libero la Ventana de Presentación (sola se libera despues de 500ms)
...
begin
  ...
  // habilito el botón para indicar que está habilitada la conexión auto
  ...
  // Chequeo los parámetros de la conexión automática y habilito la timer para la conexión
  ...
  // Si está habilitada la conexión por internet desabilito todas la paginas.
  if (Mercury.TipoDeComm = 2) then begin
...
begin
    ...
    with Equipo.CalcParam.Parametros[i] do begin
      if Calcular = 1 then begin
        ...
        for j:=0 to Nparam-1 do begin
          // me aseguro que el canal esté habilitado
          if (AParam[j].canal >= 0) then begin
            if (Equipo.Canales[AParam[j].canal].Config > 0) then
              ...
              // Si falta un parámetro que no es necesario no importa
      ...
      // Muestro la info de los parámetros calculados cuando es necesario
      if (Calcular = 1) then begin
        case i of
  ...
  and (not Equipo.ThreadComm.ConfigEquipo)) then begin
    ...
    TimerCierre.Enabled := true;       // Despues de 50ms se cierra la aplicación.
  end;
...
begin
  ...
  // Desabilito el boton para configurar si el equipo no está conectado
...
begin
  ...
  if (i>1) and (NCanal>7) and (ListaSensores[i].Entrada<>'PULSO')then begin
    // Cartel de Error
    MessageBox(Handle,'El sensor seleccionado corresponde a un canal ANALÓGICO.',
               PChar(Caption), MB_OK	or MB_ICONERROR );
    cbSensores.Visible := False;
    exit;
  end;
...
begin
  ...
  if Equipo.ThreadComm.pTmuestreo^ <> Equipo.ThreadComm.T then begin
    MessageBox(Handle,'Al cambiar el periodo de muestreo se debe re-configurar los parámetros '+
              #13+'de conexión por internet, en caso de usarse.' , PChar(Caption), MB_OK	or MB_ICONINFORMATION );
  end;

{  // La configuración de cada canal ya fue cargada cuando se seleccionan el combobox
...
begin
  // Adapto el sistema para el formato elejido (español - ingles)
...
begin
  ...
  if not Mercury.Grabando then begin
    ...
    // Guardo los parámetreos de Monitoreo
...
begin
  ...
  // Configuro los parámetros según el formato elegido
  case Mercury.FormatoReporte of
    ...
    // CSV - Planilla de cálculo (formato en español)
    1 : begin
          sep := ';';
          ext := '.txt';
        end;
    // CSV - Planilla de cálculo (formato en ingles)
    2 : begin
          sep := ',';
          ext := '.txt';
        end;
  ...
  // Genero el nombre del archivo según el tipo de archivo elejido
  ...
  if (upcase(Mercury.ReporteSel)='S') then begin
    try // Guardo la info en el archivo
      ...
      if FileExists(Archivo) then Append(F)
        ...
        Writeln(F,'Descripción de los Canales');
        Writeln(F,'--------------------------');
...
begin
  ...
  for i:=0 to PaginaWeb.Count-1 do begin
    ...
    // Informarcíon de los valores de los canales
    ...
    // Informarcíon de los valores calculados
...
begin
  ...
  if not InputQuery('Cambiar Descripción del Canal '+intToStr(Canal),
                    'Ingrese una nueva Descripción',NuevaDesc) then exit;
...
//  FCalculoParam.Caption    := 'Cálculo de parámetros - ' + (sender as TMenuItem).Caption;
...
begin
  ...
  if Mercury.ConexTelefon.AConexiones[Mercury.ConexAuto.indexConex].Select = 'S' then begin
    ...
    // Prueba de conexión - Generación del archivo de logeo
...
begin
  ...
  // Habilito el botón y el menú para desconectar
...
begin
  // Deshabilito el botón y el menú para desconectar
  ...
  try
    ...
    with mHistorialInternet.Lines do begin
      if Count>500 then begin
        ...
        Delete(0);
        // Modifico la última linea para que el curso quede sobre la última linea
